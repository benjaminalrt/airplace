...
export class Flight_tickets_nft {
    ...
    async deploy(owner: att.Address, params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./contracts/flight_tickets_nft.arl", {
            owner: owner.to_mich()
        }, params)).address;
        this.address = address;
    ...
    async mint(tow: att.Address, tid: att.Nat, tmd: Array<[
        ...
        if (this.address != undefined) {
            return await ex.call(this.address, "mint", mint_arg_to_mich(tow, tid, tmd), params);
        }
        throw new Error("Contract not initialised");
    }
    ...
    async get_mint_param(tow: att.Address, tid: att.Nat, tmd: Array<[
        ...
        if (this.address != undefined) {
            return await ex.get_call_param(this.address, "mint", mint_arg_to_mich(tow, tid, tmd), params);
        }
        throw new Error("Contract not initialised");
    }
    ...
    async get_operator_value(key: operator_key): Promise<operator_value | undefined> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            const data = await ex.get_big_map_value(BigInt(att.Int.from_mich((storage as att.Mpair).args[5]).toString()), key.to_mich(), operator_key_mich_type);
            if (data != undefined) {
    ...
    async get_operator_for_all_value(key: operator_for_all_key): Promise<operator_for_all_value | undefined> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            const data = await ex.get_big_map_value(BigInt(att.Int.from_mich((storage as att.Mpair).args[6]).toString()), key.to_mich(), operator_for_all_key_mich_type);
            if (data != undefined) {
...
export const flight_tickets_nft = new Flight_tickets_nft();
