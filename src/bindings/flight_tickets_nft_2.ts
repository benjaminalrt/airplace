...
export class Flight_tickets_nft_2 {
    ...
    async deploy(owner: att.Address, params: Partial<ex.Parameters>) {
        const address = (await ex.deploy("./contracts/flight_tickets_nft_2.arl", {
            owner: owner.to_mich()
        }, params)).address;
        this.address = address;
    ...
    async get_royalties_value(key: att.Nat): Promise<Array<part> | undefined> {
        if (this.address != undefined) {
            const storage = await ex.get_raw_storage(this.address);
            const data = await ex.get_big_map_value(BigInt(att.Int.from_mich((storage as att.Mpair).args[5]).toString()), key.to_mich(), royalties_key_mich_type);
            if (data != undefined) {
...
export const flight_tickets_nft_2 = new Flight_tickets_nft_2();
